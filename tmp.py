...
def events_to_voxel_grid(events, num_bins, width, height):
    ...
    # print('events: ', events)
    # print('first, start: ', first_stamp, last_stamp)
    # print('delta: ', deltaT)
    ...
    assert all(pol == 1 or pol == -1 for pol in pols)

    tis = ts.astype(np.int64)
    valid_indices = tis < num_bins
    print('tis: ', tis)
    print('len: ', len(valid_indices))
    print('len True: ', np.count_nonzero(valid_indices))
    ...
    #     t_bin = int(np.clip(((timestamp - first_stamp) / deltaT) * (num_bins-1), 0, num_bins - 1))
...
def save_voxel(file_name):
    # for num_bin in num_bins:
        ...
        # print('bag loaded')
        input = events_to_voxel_grid2(input, 150, SIZE_X, SIZE_Y)
        # print('to voxel')
