...
rosbag_files = glob.glob("./Dataset/rosbag_tmp/*.bag")
srcpath = glob.glob("./Dataset/src_tmp/*.txt")[0]
...
def events_to_voxel_grid2(events, num_bins, width, height):
    ...
    for event in events:
        ...
        t_bin = int(np.clip((timestamp / events[-1, 0]) * num_bins, 0, num_bins - 1))
    ...
    #     assert voxel_grid[y-1][x-1] != 1 or voxel_grid[y-1][x-1] == 1, 'unexpected'
    
    # for frame in voxel_grid:
    #     coordinates = nonzero_coordinates(frame)[:, [1, 0]]
    #     for coord in coordinates:
    #         if coord not in events[:, 1:3]:
    #             print('unexpected')
    #         else:
    #             print(coord)
    # print('voxel grid: ', voxel_grid[0])
    # print('voxel grid: ', voxel_grid[10])
    return voxel_grid
...
if __name__ == "__main__":
    # shutil.rmtree('./Dataset/target_data_tmp/')
    # os.mkdir('./Dataset/target_data_tmp/')
    for num_bin in num_bins:
        for rosbag_file in rosbag_files:
            name_input = rosbag_file.replace('rosbag', 'input_dataaaa').replace('.bag', f'_{num_bin}.npz')
            ...
            if not os.path.exists(name_input):
                ...
                input = events_to_voxel_grid2(input, num_bin, SIZE_X, SIZE_Y)
                input = np.array(input, dtype=np.int32)
                input_norm = min_max(input)
                # print('normalized: ', input_norm[0])
                # print('normalized: ', input_norm[10])
                break
        ...
        while True:
            ...
            np.savez_compressed(f'./Dataset/target_data_tmp/{data_name}_{num_bin}.npz', result_coords)
