...
class MultiBranchNetwork(nn.Module):
    ...
    def forward(self, x1, x2):
        output1 = self.unet1(x1)
        output2 = self.unet2(x2)
        combined_output = self.combine_tensor(output1, output2)
...
class ResNet18(nn.Module):
    def __init__(self, block, layers, se=False, **kwargs):
        ...
        self.additional_conv = nn.Conv3d(
            in_channels=self.base_channel // (1 if self.low_rate else self.alpha), # 最初の層の出力チャネル数を入力チャネル数として使用
            out_channels=self.base_channel // (1 if self.low_rate else self.alpha), # 出力チャネル数も同じに設定（必要に応じて変更）
            kernel_size=(5, 7, 7),
            stride=(1, 2, 2),
            padding=(2, 3, 3),
            bias=False
...
class HighRateBranch(ResNet18):
    ...
    def forward(self, x):
        laterals = []
        x = x.transpose(1, 2) # 8x1x8x320x320
        x = self.conv1(x) #8x8x8x160x160
        x = self.bn1(x)

        x = self.additional_conv(x) # add conv layer
        x = self.bn1(x)
...
class LowRateBranch(ResNet18):
    ...
    def forward(self, x, laterals):
        ...
        x = self.maxpool(x)
         ...
         # xのサイズを変更（リサイズ）
        desired_size = laterals[0].size()[2:]  # 期待するサイズ（空間次元のみ）
        x = F.interpolate(x, size=desired_size, mode='trilinear', align_corners=False)

        x = self.mfm1(laterals[0], x)
        downsample = nn.AvgPool3d(kernel_size=2, stride=2)
        laterals_resized = downsample(laterals[0])

        # サイズ確認
        # print("Size of x after mfm1:", x.size())
        # print("Size of laterals_resized:", laterals_resized.size())

        # サイズの一致を確認して調整
        if x.size()[2:] != laterals_resized.size()[2:]:
            # サイズが異なる場合、適切にリサイズする
            laterals_resized = F.interpolate(laterals_resized, size=x.size()[2:], mode='trilinear', align_corners=False)

        x = torch.cat([x, laterals_resized], dim=1)

        # x = torch.cat([x, laterals[0]], dim=1) # (b, 80, 30, 22, 22) # ここでダウンサンプリングして形を合わせている
...
class MSTP(nn.Module):
    def __init__(self, args, dropout=0.5):
        ...
        self.v_cls = nn.Linear(1024*2, 2) # 出力サイズを2に設定
